...
Your website must somehow dynamically load information from the web API you've chosen and present information from that response on the page. This requires that you must:
  * Make at least **two** (2) `fetch` requests in response to 2 distinct UI events that are not the `load` event (in other words, both requests must be in response to an event other than `load`). Additionally, **both** fetch requests must be in response to a UI event on a different element (in other words, it will not count if you make two fetch requests when one button is clicked).
    * These fetch requests must be to **distinct** endpoints (keep in mind that query parameters do not make an endpoint *distinct*)
    * Not all of the APIs listed above have two (2) endpoints. You can use endpoints from multiple APIs to meet the requirements.
    * The same event can be used (for example, two clicks) but each should be attached to different element on your page
